...
def run_rollout(data_path, controller_type, model_path, debug=False):
  ...
  #controller = importlib.import_module(f'controllers.{controller_type}').Controller(data_path=str(data_path))
...
if __name__ == "__main__":
  ...
  if data_path.is_file():
    ...
    results = process_map(run_rollout_partial, files, max_workers=1, chunksize=1)
